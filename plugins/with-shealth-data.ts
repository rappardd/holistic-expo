import {
  ...
  const AAR_NAME = "samsung-health-data-api"; // rename to actual filename (no .aar)
  ...
  const withSHealthGradle: ConfigPlugin = (config) => {
    return withAppBuildGradle(config, (cfg) => {
      ...
      // packagingOptions for duplicate META-INF (common with vendor AARs)
      gradle.contents = gradle.contents.replace(
        /android\s*{([\s\S]*?)}/m,
        (block) =>
  ...
  const withSHealthManifest: ConfigPlugin = (config) => {
    return withAndroidManifest(config, (cfg) => {
      ...
      // Adjust the list to the types your app needs.
      // Example for steps + heart rate read, and exercise write:
